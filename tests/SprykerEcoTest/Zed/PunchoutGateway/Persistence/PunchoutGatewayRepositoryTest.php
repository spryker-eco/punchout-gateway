...
class PunchoutGatewayRepositoryTest extends Unit
{
    ...
    {
        $storeTransfer = $this->tester->haveStore([StoreTransfer::NAME => 'DE']);
        $connectionA = $this->tester->havePunchoutConnection(['fk_store' => $storeTransfer->getIdStoreOrFail()]);
        $connectionB = $this->tester->havePunchoutConnection(['fk_store' => $storeTransfer->getIdStoreOrFail()]);
    ...
    public function testGetPunchoutCredentialCollectionExcludesNotIdPunchoutConnections(): void
    {
        $storeTransfer = $this->tester->haveStore([StoreTransfer::NAME => 'DE']);
        $connectionToExclude = $this->tester->havePunchoutConnection(['fk_store' => $storeTransfer->getIdStoreOrFail()]);
        $connectionToInclude = $this->tester->havePunchoutConnection(['fk_store' => $storeTransfer->getIdStoreOrFail()]);

        $excludedCredential = $this->tester->havePunchoutCredential([
            'fk_punchout_connection' => $connectionToExclude->getIdPunchoutConnectionOrFail(),
        ]);
        $includedCredential = $this->tester->havePunchoutCredential([
            'fk_punchout_connection' => $connectionToInclude->getIdPunchoutConnectionOrFail(),
        ]);

        $criteriaTransfer = (new PunchoutCredentialCriteriaTransfer())
            ->setNotIdPunchoutConnections([$connectionToExclude->getIdPunchoutConnectionOrFail()]);
